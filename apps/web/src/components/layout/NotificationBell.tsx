  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <style jsx global>{`
        ...
        .bell-ring-active {
          animation: bellRingWiggle 1.8s infinite ease-in-out;
      ...
      <button
        ...
        className="relative p-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition focus:outline-hidden group"
        title={isConnected ? "การแจ้งเตือน Realtime (ออนไลน์)" : "การแจ้งเตือน (Notifications)"}
        aria-label="การแจ้งเตือน"
      >
        <Bell className={`w-5 h-5 transition-transform duration-200 group-hover:scale-110 ${unreadCount > 0 ? 'bell-ring-active text-theme-primary' : ''}`} />
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 flex items-center justify-center">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75" />
            <span className="relative flex items-center justify-center min-w-4 h-4 px-1 text-[10px] font-bold text-white bg-rose-600 rounded-full ring-2 ring-white shadow-xs">
      ...
      {/* Dropdown Popover */}
      {isOpen && (
        <div className="fixed inset-x-3 top-16 sm:absolute sm:inset-x-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-96 max-w-sm mx-auto sm:mx-0 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          {/* Header */}
